...
#### TCP与UDP
  ...
  **UDP**的全称是**用户数据报协议**，在网络中它与TCP协议一样用于处理数据包，是一种无连接的协议。在OSI模型中，在传输层，处于IP协议的上一层。UDP有不提供数据包分组、组装和不能对数据包进行排序的缺点，也就是说，当报文发送之后，是无法得知其是否安全完整到达的。有以下的特点：
...
#### WebSocket
  ...
  ***总结：***
  ...
  WebSocket 是现代 Web 应用中实现高效、低延迟、双向实时通信的通信协议。它通过基于 HTTP 的握手建立连接，然后切换到轻量级的二进制帧协议进行全双工数据传输，完美解决了传统 HTTP 轮询/长轮询/流技术在实时性、效率和双向性方面的不足。在开发聊天、协作、游戏、实时数据监控等应用时，WebSocket 通常是首选方案。理解其协议细节（握手、帧结构、控制帧）和 API 使用，并注意安全性、心跳和错误处理，对于构建健壮的实时应用至关重要。
